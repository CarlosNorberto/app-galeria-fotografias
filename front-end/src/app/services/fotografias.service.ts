...
export class FotografiasService {
  ...
  getFotografiasAdmin(token:string){
    ...
    return this._http.get(this.url + 'fotografias-admin',options)
    .toPromise().then(res=>res.json());
  }
  ...
  save(fotografia:any, token:string){
    ...
    let options=new RequestOptions({headers:headers});
    return this._http.post(this.url + 'fotografia',fotografia, options)
    .toPromise().then(res=>res.json());
  }
  ...
  update(id:number, fotografia:any, token:string){
    ...
    let options=new RequestOptions({headers:headers});
    return this._http.put(this.url + 'fotografia/' + id,fotografia, options)
    .toPromise().then(res=>res.json());
  }
